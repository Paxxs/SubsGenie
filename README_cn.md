...
SubsGenie 是一个为 Clash（Mihomo）代理订阅自动更新和合并的工具，通过 GitHub Actions 自动化运行。它通过预先处理和合并多个订阅源，并将结果保存到 GitHub Gist，从而显著减少客户端（如 Shadowrocket、Clash 等）获取订阅时的等待时间。

[English](README.md)

## 特性

- **自动化合并与更新**：通过 GitHub Actions 定时运行，自动获取并合并多个订阅源，无需手动操作。
- **减少客户端等待时间**：预处理订阅内容并存储到 Gist，使客户端获取时几乎无需等待，避免像 iOS 上的 Shadowrocket 因等待时间过长而失败的问题。
- **多源订阅支持**：支持设置主要订阅 URL 和额外订阅 URL，轻松整合多个来源的节点。
- **Gist 存储与分发**：将处理后的订阅文件存储在 GitHub Gist 上，既保证了隐私性又提供了可靠的访问点。
- **反向代理支持**：提供 Cloudflare Workers 反向代理功能，解决某些地区无法直接访问 GitHub Gist 的问题。
- **灵活配置**：通过环境变量进行详细控制，根据需要调整合并和处理过程。
...
SubsGenie：自动化管理你的 Clash (Mihomo) 代理订阅，让你的网络体验更加流畅稳定。
